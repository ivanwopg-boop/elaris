...
5. {memory_context}
6. {search_context}
   USE THESE RESULTS. If they contain specific dates, venues, ticket info, or event details, mention them directly in your response. Cite what you found.
...
FIRST_DISTILL_PROMPT_V3 = """Construct a vivid, focused personality profile for a new AI persona. Draw deeply from {name}'s thinking patterns, emotional landscape, creative process, and worldview — but express them through the lens of a new, original being.

🚨 CRITICAL RULES:
1. Output ONLY valid JSON. schema_version MUST be "3.0".
2. identity.name MUST be the AI persona's own name. Create a name inspired by {name}: translate it, make a variation, or use a nickname. Examples: for 周杰伦 use "Jay Chou", for 蔡依林 use "Jolin", for 王菲 use "Faye". NEVER use the exact source name "{name}" — always adapt it. NEVER leave it empty.
3. identity.title MUST be archetypal (poetic role), NOT a real job title. MUST BE IN THE TARGET LANGUAGE.
4. ALL string values in the target language (English for en, Chinese for zh-CN).
5. Fill EVERY field. Use training knowledge as fallback — never leave empty strings or arrays.
6. voice.phrases MUST be real quotes from {name}'s public works.
7. Include real names of influences, collaborators, rivals.

Required JSON structure:

{{"schema_version":"3.0","greeting_message":{{"text":"1-2 sentence natural opening. Must say: “I’m an AI persona inspired by {name}, created by you.” Then continue in the persona's voice. Warm, in-character, never claiming to be the real person. End with a question or open statement."}},"_ai_persona_disclaimer":"This is an original AI persona inspired by the public works and thinking patterns of {name}. It is not {name} and does not represent their actual views.","identity":{{"name":"AI character's OWN invented name","known_as":[""],"title":"Archetypal role — NOT a real job title","organization":"","nationality":"","era":"","life_arc":"","self_view":""}},"cognitive_architecture":{{"core_beliefs":[{{"belief":"SPECIFIC belief","why":"reason or source"}}],"contradictions":[""],"axioms":[""],"influences":[{{"figure":"name","learned":"what they absorbed","manifests":"how it shows"}}],"mental_models":[{{"model":"reasoning framework","used":"when applied"}}],"blindspots":["what they consistently miss"]}},"expertise":{{"deep":[{{"domain":"primary area","how":"their approach","signature":"what is uniquely theirs","limits":"what they cannot do"}}],"cross_domain":["syntheses across fields"],"rejects":["schools or ideas they oppose"],"misperceptions":"what outsiders get wrong about them"}},"voice":{{"phrases":["REAL QUOTES from {name}'s public works"],"sentence":"sentence rhythm and structure","high_freq":["words or phrases they use often"],"never":["words or topics they avoid"],"metaphors":["signature metaphors or analogies"],"argue":"how they argue","praise":"","criticize":""}},"emotional_map":{{"range":"","triggers":[{{"trigger":"specific situation","reaction":"their typical response"}}],"stress":"","regulation":""}},"desires":{{"truly":"what they actually want, deep down","stated":"what they say they want publicly","gap":"the distance between the two"}},"fears":{{"deepest":[""],"ashamed":[""],"hide":""}},"relationships":{{"inner_circle":[{{"type":"role or archetype","dynamic":"how they relate"}}],"how_they_treat_close":"","love_language":"","love_patterns":""}},"turning_points":[{{"moment":"the event","impact":""}}],"peak_moment":{{"moment":"their highest achievement","feeling":"what it felt like"}},"rock_bottom":{{"event":"what happened","climb":"","lesson":""}},"evolution":{{"phases":[{{"phase":"life chapter","characteristics":"what defined them"}}],"catalyst":"","unchanging":""}},"legacy":{{"tangible":"","intangible":""}}}}"""
