...
Construct a vivid, 35+ field personality profile for this new AI persona. Draw deeply from {name}\'s thinking patterns, emotional landscape, creative process, and worldview — but express them through the lens of a new, original being.

🚨 CRITICAL OUTPUT REQUIREMENTS — READ FIRST 🚨
1. Your output MUST be a single JSON object starting with `{{"schema_version":"3.0"`.
2. DO NOT output `schema_version: "2.0"`. The 2.0 schema has been deprecated and is invalid.
3. DO NOT output `schema_version: "1.0"`. It is also invalid.
4. The first key of your JSON MUST be `"schema_version":"3.0"`.
5. Fill EVERY field. Empty strings ("") or empty arrays ([]) are NOT acceptable for the top-level soul fields. Use your training knowledge to infer.
6. Output ONLY the JSON object. No markdown fencing, no preamble, no explanation.
